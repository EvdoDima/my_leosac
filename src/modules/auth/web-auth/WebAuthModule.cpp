...
WebAuthModule::WebAuthModule(zmqpp::context &ctx,
        zmqpp::socket *pipe,
        ...
        BaseModule(ctx, pipe, cfg, utils)
{
    ...
    for (auto authenticator : authenticators_)
    {
        reactor_.add(authenticator->bus_sub(),
                std::bind(&WebAuthInstance::handle_bus_msg, authenticator));
    }
}

WebAuthModule::~WebAuthModule()
{
}

void WebAuthModule::process_config()
{
    ...
    for (auto &node : module_config.get_child("instances"))
    {
        ...
        INFO("Creating WebAuth instance " << auth_ctx_name << ". Target door = " << auth_target_name);
        authenticators_.push_back(WebAuthInstancePtr(new WebAuthInstance(ctx_,
                auth_ctx_name,
                auth_sources_names,
                auth_target_name,
                config_file,
...
void WebAuthModule::dump_additional_config(zmqpp::message *out) const
{
    assert(out);
